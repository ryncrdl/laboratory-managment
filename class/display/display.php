...
class display
{
	...
	public function display_member()
	{
		...
		if ($count > 0) {
			foreach ($fetch as $key => $value) {
				...
				$data['data'][] = array($value['m_school_id'], $value['m_fname'] . ' ' . $value['m_lname'], $value['m_gender'], $value['m_contact'], $value['m_department'], $value['m_year_section'], $value['m_type'], $button, $value['m_fname'], $value['m_lname'], $value['id'], $value['m_email']);
			}
			echo json_encode($data);
		} else {
			$data['data'] = array();
    ...
    ');
		...
		if ($count > 0) {
			foreach ($fetch as $key => $value) {
				$button = "<button class='btn btn-primary roomreserve' data-id='" . $value['reservation_code'] . "'>
							Reserve
							<i class='fa fa-chevron-right'></i>
							</button>";
				$date = date('F d, Y H:i:s A', strtotime($value['reserve_date'] . ' ' . $value['reservation_time']));
...
switch ($key) {
	...
	case 'accept_reservation';
		...
		break;
	case 'accept_room_reservation';
		$display->accept_room_reservation();
		break;
